...
class TranscriptionWorker:
    ...
    def run(self):
        ...
        try:
            ...
            error_msg = str(e)
            if "Unable to open file 'model.bin'" in error_msg or "model.bin" in error_msg:
                model_cache_path = self.model_path if os.path.isabs(self.model_path) else None
                if model_cache_path and os.path.exists(os.path.dirname(model_cache_path)):
                    cache_dir = os.path.dirname(model_cache_path)
                    logging.error(
                        f"模型文件损坏或下载不完整。请删除以下目录后重新运行程序：\n"
                        f"  {cache_dir}\n"
                        f"或者删除整个模型缓存目录：\n"
                        f"  {os.path.dirname(os.path.dirname(os.path.dirname(cache_dir)))}\n"
                        f"程序将自动重新下载模型。"
                    )
                else:
                    logging.error(
                        f"模型下载失败或文件损坏。请检查网络连接，或手动删除 Hugging Face 缓存目录后重试：\n"
                        f"  C:\\Users\\<用户名>\\.cache\\huggingface\\hub\\models--Systran--faster-whisper-tiny"
                    )
...
class AudioToTextRecorder:
    ...
    def __init__(self,
        ...
        # Add timeout to prevent infinite waiting, with periodic process checks
        timeout = 300  # 5 minutes timeout
        check_interval = 1.0  # Check process status every second
        elapsed = 0.0
        
        while not self.main_transcription_ready_event.wait(timeout=check_interval):
            elapsed += check_interval
            # Check if the process is still alive
            if hasattr(self.transcript_process, 'is_alive') and not self.transcript_process.is_alive():
                raise RuntimeError(
                    "Transcription process died during initialization. "
                    "Check logs for error details. This may indicate a problem with model loading or dependencies."
                )
            if elapsed >= timeout:
                raise RuntimeError(
                    f"Transcription model initialization timed out after {timeout} seconds. "
                    "The model may be downloading or initializing. Please check your network connection and try again."
                )
